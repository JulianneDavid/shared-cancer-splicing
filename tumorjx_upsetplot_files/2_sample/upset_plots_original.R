...
lr <- make.ggset.final(setsizes.data = wholeset, subsetsizes.data = subset, 
                       ...
                       subsetplot.dims = unit(c(0.1, 2.68, -1.65, 0.78), 'cm'),
...
# make the axis labels
{
  ...
  grplabels = c("Oocyte", "Myoblast", "Ectoderm", "Placenta", "Zygote", "Neonate Fetal", "Stem Cells",
                "Developmental", "Other Embryo", "Other Adult", "Unexplained")
  
  cp <- make.ggset.final(setsizes.data = wholeset, 
                         subsetsizes.data = subset,
                         hl.name = "unexplained",
                         setsize.grplabels = grplabels,
